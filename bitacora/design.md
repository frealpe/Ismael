...
> decisiones por feature* van en `—`; los *patrones de código* en `—`;
> las *trampas de trabajar aquí* en `memory.md`. Tope objetivo ~200 líneas: si una decisión crece, se
> extrae a su tema de `—` y aquí queda 1-3 líneas + link.
...
> Decisiones grandes y transversales. Formato corto y fechado; el detalle va a `—`.

### <YYYY-MM-DD> — <Título de la decisión>
- **Contexto:** el problema en 1-2 líneas.
- **Decisión:** qué se eligió.
- **Alternativas descartadas:** X (por qué no), Y (por qué no).
- **Consecuencias / detalle:** link al tema de `—` si aplica.
...
Dónde vive cada tipo de información: ver `guia-agente.md` → "Dónde vive cada cosa" (no duplicar aquí).
