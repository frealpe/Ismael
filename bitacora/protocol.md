...
> Los rituales condensados están en `Ismael/SKILL.md`; el *por qué*, en `Ismael/guide.md`.
...
5. Nada de esto vive en memoria privada del agente; todo en el proyecto. **Tampoco los artefactos**
   —scripts de un solo uso, extracciones, volcados—: su hogar es `artefactos/sesion-{NNN}/`, y
   esa regla vale por encima de cualquier default del harness (`Ismael/SKILL.md` → Precedencia).
6. **Un hogar por dato** (mapa en `Ismael/SKILL.md` y en `guia-agente.md`).
...
dentro de `history/`); los que apuntan fuera del bloque se rompen y hay que revisarlos.

## Archivos y su rol

Ver `Ismael/guide.md` → "Roles y fronteras". Aquí solo los **formatos**.

### `latest.md` — formato fijo, se SOBREESCRIBE (~{{budget:state}} líneas)
...
### `—` — tabla append-only (OPCIONAL, feature `effort_log`)
...
en el `sesion-{NNN}-{YYYY-MM-DD}.md` de la sesión que auditó, y lo que perdura, en el hogar que corrigió.
...
`git log --diff-filter=A -- history/<archivo de sesión>`.
...
(`cartas.md`, como `index.md`), un archivo por carta (`carta-{NNN}-{YYYY-MM-DD}.md`, como `sesion-{NNN}-{YYYY-MM-DD}.md`:
inmutable, numerado, se lee con grep) y su carpeta (`correspondencia/`). Lo crea la primera
...
### `artefactos/` — artefactos por sesión (se crea con el primero, no en bootstrap)

Un subdirectorio por sesión: `artefactos/sesion-{NNN}/`. Dentro va lo que la sesión produjo y
...
**Dos clases, y el `sesion-{NNN}-{YYYY-MM-DD}.md` dice cuál es cuál.** Un artefacto que **sostiene un cambio
...
### `handover.md` — checkpoint de trabajo en curso (~{{budget:handover}} líneas)

Estados: `SIN_TRABAJO_ACTIVO` | `EN_PROGRESO` | `COMPLETADO`. **Regla dura:** antes de un cambio interrumpible,
`EN_PROGRESO` con objetivo/alcance/verificación **y sello**. Al cerrar, siempre refrescar el puntero a
la sesión que se cierra AHORA. Plantilla en `Ismael/core/templates/handover.md`.
...
`—<TEMA>.md` y aquí queda un resumen de 2-3 líneas + link; si un tema supera ~600-800
líneas, se divide en sub-temas. `—` se crea **cuando la primera decisión cruce el
...
**Es el complemento de `index.md`, y por eso se lee en cada arranque y el historial no.** El
...
**No la guardes en `handover.md`**: ese doc se poda por diseño, y ahí la trampa muere sin aviso.
Caso real de campo.
...
hay nada que hacer: los detectores de `Ismael/core/rituals/auditar.md` ya sirven tal cual.
...
sea—, la regla vive en `guia-agente.md` → Convenciones. Lo que va aquí es **cómo se comprueba**, porque una
...
tail -n 1 history/index.md | LC_ALL=C grep -n '[^ -~]'
...
dispara **y** que estás en la semántica que el patrón supone. Ver `Ismael/core/reference/verificar.md` →
...
while IFS='|' read -r _ rol tope _; do
  ...
  case "$rol" in
    entry) f=guia-agente.md ;;    gotchas)  f=memory.md ;;
    state) f=latest.md ;;    handover) f=handover.md ;;
    *) continue ;;
  esac
  n=$(wc -l < "$f")
...
es información: es decoración*, `Ismael/core/reference/verificar.md`— aplicada a un detector que
...
Condensados en `Ismael/core/rituals/` (un archivo por ritual). Este archivo es la referencia de
